...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (ai === null) {
      log.warn("AI classifier unavailable; refusing to report low risk");
      return new Response(JSON.stringify({ risk_level: "unavailable", source: "keyword_only" }), {
        status: 503,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    const finalRisk = mergeRisk(ai, kw);
    ...
    return new Response(JSON.stringify({ risk_level: "unavailable", error: "screening_unavailable" }), {
      status: 503,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
